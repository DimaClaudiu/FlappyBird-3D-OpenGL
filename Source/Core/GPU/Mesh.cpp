...
#include <Laboratoare/Tema1/Flappy.h>
...
void Mesh::Render(int defaultIndex) const
{
	...
	for (unsigned int i = 0; i < meshEntries.size(); i++)
	{
		if (useMaterial)
		{
			...
			if (materialIndex != INVALID_MATERIAL && materials[materialIndex]->texture)
			{
				...
				TextureManager::GetTexture(static_cast<unsigned int>(defaultIndex))->BindToTextureUnit(GL_TEXTURE0);
			}
		}
